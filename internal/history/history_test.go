...
func TestLogFormat(t *testing.T) {
	...
	data, err := os.ReadFile(filepath.Join(dir, ".certui", "history.log"))
	if err != nil {
...
func TestLogDirCreation(t *testing.T) {
	...
	logPath := filepath.Join(dir, ".certui", "history.log")
...
func TestRead(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(dir, ".certui"), 0755); err != nil {
		t.Fatal(err)
	}
	content := "line1\nline2\nline3\nline4\nline5\n"
	if err := os.WriteFile(filepath.Join(dir, ".certui", "history.log"), []byte(content), 0644); err != nil {
		t.Fatal(err)
	}
